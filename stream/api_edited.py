...
if __name__ == "__main__":
    ...
    # workload = 'inputs/examples/workload/resnet18.onnx'
    ...
    scme, _ = get_hardware_performance_stream(accelerator, workload, mapping, CN_define_mode, hint_loops, node_hw_cost_pkl_name)
    print(scme)
    ...
    plot_timeline_brokenaxes(scme[0].workload, scme[0].accelerator, draw_dependencies, section_start_percent,
                             percent_shown, plot_data_transfer, fig_path=timeline_fig_path, show=show)
    plot_memory_usage(scme[0].accelerator.memory_manager, fig_path=memory_fig_path, show=show)
    bar_plot_stream_cost_model_evaluations_breakdown([scme], fig_path=energy_fig_path, show=show)
    # save_graph(scme[0].workload, "outputs/graph.svg", all_ranges=True)

    first_node = list(scme[0].workload.nodes)[1]
